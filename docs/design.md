...
After an interaction, the next observation waits until the DOM has been quiet for 50 ms and no started, finite animation runs: at least 32 ms, at most 400 ms. A persistent MutationObserver in the page records the last mutation time. A decision made during a closing dialog's transition would otherwise go stale and cost a model call. Editable ARIA comboboxes instead wait for visible options, capped at 200 ms. A script installed on every document counts the page's fetch/XHR requests in flight and marks a started navigation. The post-click wait also waits for them within its 400 ms cap. An explicit WAIT returns when the page has changed, the DOM has been quiet for 100 ms, and the network has been idle for 200 ms. It returns after 500 ms if nothing changes and the network is idle, and waits up to 10 s while requests are in flight.

The snapshot offers a control only when `cache.hit` finds a viewport point that hit-tests to it: its centre, a fragment centre, or a small grid. The executor clicks through the same function, so an offered control is one input can reach. A checkbox or radio that is clipped, transparent, or off-screen is operated through its visible label. Typing checks that the observed field still holds focus before Select All and again before insertion. A lost focus is a non-retryable error, because the click already ran. Date, time, datetime-local, month, and week inputs are assigned an ISO value, like a dropdown, and the text helper receives the field's `format`.

Stopping takes a second look. A DONE below 0.7 operation confidence must repeat on the next decision. A first BLOCKED waits for the page to change, then decides again. `done_when(page)` lets calling code end a run without a model call. The no-progress stop counts an unobserved outcome (`page_changed=None`) as no progress. The waits poll on timers, because a background tab can stop producing animation frames. Network loading is never fast-forwarded in the recording.
