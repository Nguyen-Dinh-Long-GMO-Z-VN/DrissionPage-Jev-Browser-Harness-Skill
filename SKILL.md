...
- `jev_run(goal, max_steps=20, done_when=None)` runs the full loop on the current tab until `done` or `blocked`. Pass `url=` to use
  a dedicated window instead. Returns `{status, reason, done_by, url, title, steps, model_calls, text_calls,
  text_prefetches, elapsed_ms, history, unverified_text}`. Pass `done_when=lambda page: ...` (the page has `url`,
  `title`, `text`, `actions`) when your code can recognise the outcome: the run then ends without asking the model. `max_steps` caps executed actions and model calls (twice that), so a stuck run cannot drain a small
...
- This repo ships `domain-skills/facebook/` (bulk-scraping a Group's posts) and `domain-skills/youtube/` (a video's
  transcript from the `get_transcript` response). Copy a folder into `$BH_AGENT_WORKSPACE/domain-skills/` to use it.
