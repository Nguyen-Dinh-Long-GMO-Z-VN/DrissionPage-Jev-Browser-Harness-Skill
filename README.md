...
| Helper | What it does |
| --- | --- |
| `jev_run(goal, max_steps=20, done_when=None)` | Runs the full loop on the current tab until `done` or `blocked`. `done_when(page)` lets your code end the run as done without a model call. Returns status, reason, `done_by` (`model` or `code`), URL, steps, model calls, text-helper calls and prefetches, elapsed time, history, and `unverified_text` (fields whose typed text could not be read back). |
...
- **Validated targets.** A control is offered only if a click can land on it: its centre, a fragment of a wrapped link, or a point of a small grid must hit-test to the control itself. A control clipped by a scroll container or under a banner is not offered. Before input, clicks check the document, form values, target, and nearby context, then resolve current geometry and hit-test with the same function. A checkbox or radio styled away (clipped, transparent, off-screen) is clicked through its visible label. Typing stops if a page handler moves focus to another field. Date, time, and month inputs take ISO values, and the text helper is told the format.
- **Second votes for stopping.** A `DONE` below 0.7 confidence must be chosen twice in a row. A first `BLOCKED` waits for the page to change, then decides again, because half-loaded pages and opening menus are the usual cause. Three actions in a row without a proven page change stop the run, including when an observation failed.
- **Waits for useful state.** After typing into a combobox, wait up to 200 ms for suggestions. Other interactions wait until the DOM has been quiet for 50 ms and no finite animation runs (at least 32 ms, at most 400 ms), so a closing dialog does not produce a decision that goes stale. A script counts the page's own fetch/XHR requests in flight. After a click the settle also waits for short requests, still within 400 ms. `WAIT` returns once the page has changed, the DOM has been quiet for 100 ms, and the network has been idle for 200 ms. It returns after 500 ms if nothing changes and the network is idle. While requests are in flight it keeps waiting, up to 10 s. A page younger than 2.5 s must hold still for 100 ms (capped at 500 ms) before a decision.
